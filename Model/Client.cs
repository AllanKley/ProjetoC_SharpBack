...
public class Client : Person, IValidateDataObject, IDataController<ClientDTO, Client>
{
    ...
    public int save()
    {
        ...
        {
            var ExistClient = context.Client.Where(c => c.document == this.document || c.email == this.email || c.login == this.login);
            foreach (var instance in ExistClient)
            {
                if (ExistClient != null)
                {
                    if (this.email == instance.email)
                        ex.Add("email", Error.GetMessage(Errors.EmailAlreadyExists));
                    if (this.document == instance.document)
                        ex.Add("document", Error.GetMessage(Errors.DocumetnAlreadyExists));
                    if (this.login == instance.login)
    ...
    public static Client find(int id)
    {
        using (var context = new DAOContext())
        {
            ...
            var clientDAO = context.Client.Include(i => i.address).FirstOrDefault(o => o.id == id);
